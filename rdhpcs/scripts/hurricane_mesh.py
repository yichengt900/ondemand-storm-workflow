...
def main(args, clients):
    ...
    io = pathlib.Path('/lustre')

    storm_name = str(args.name).lower()
    storm_year = str(args.year).lower()
    tag = args.tag
    if tag is None:
        tag = f'{storm_name.lower()}_{storm_year}'

    logger.info(f"The simulation tag is {tag}.")

    dem_dir = pathlib.Path(io / 'dem')
    out_dir = io / 'hurricanes' / tag / 'mesh'
    ...
    if cmd == 'subset_n_combine':
        ...
        args.rasters = [i for i in (dem_dir / 'gebco').iterdir() if i.suffix == '.tif']
        args.out = out_dir 
        args.fine_mesh = io / 'grid' / 'HSOFS_250m_v1.0_fixed.14'
        args.coarse_mesh = io / 'grid' / 'WNAT_1km.14'
        args.region_of_interset = io / 'hurricanes' / tag / 'windswath'
...
class HurricaneMesher:
    ...
    def run(self, args):
        ...
        dem_dir = pathlib.Path(io / 'dem')
        shp_dir = pathlib.Path(io / 'shape')
        hurr_info = pathlib.Path(
            io / 'hurricanes' / tag / 'windswath')
        out_dir = pathlib.Path(
            io / 'hurricanes' / tag / 'mesh')
        out_dir.mkdir(exist_ok=True, parents=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--tag", "-t",
        help="storm tag used for path creation", type=str)
    parser.add_argument(
