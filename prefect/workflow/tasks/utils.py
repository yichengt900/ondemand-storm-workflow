...
@resource_manager(name="File mutex")
class FLock:
    def __init__(self, path):
        self.path = path

    def setup(self):
        "Create a locked file in the specified address and returns file object"
        file_obj = open(self.path, 'w')
        fcntl.flock(file_obj.fileno(), fcntl.LOCK_EX)
        return file_obj

    def cleanup(self, file_obj):
        "Removes the lock"
