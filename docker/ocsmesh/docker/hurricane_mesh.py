...
EFS_MOUNT_POINT = pathlib.Path('~').expanduser() / f'app/io'
...
def main(args, clients):
    ...
    out_dir = EFS_MOUNT_POINT / args.out
    ...
    if cmd == 'subset_n_combine':
        ...
        args.rasters = [
            i for i in (EFS_MOUNT_POINT / args.rasters_dir / 'gebco').iterdir() if i.suffix == '.tif'
        ]


        args.out = out_dir
        args.fine_mesh = EFS_MOUNT_POINT / args.fine_mesh
        args.coarse_mesh = EFS_MOUNT_POINT / args.coarse_mesh
        args.region_of_interset = EFS_MOUNT_POINT / args.region_of_interset
...
class HurricaneMesher:
    ...
    def run(self, args):
        ...
        dem_dir = EFS_MOUNT_POINT / args.rasters_dir
        shp_dir = EFS_MOUNT_POINT / args.shapes_dir
        hurr_info = EFS_MOUNT_POINT / args.windswath
        out_dir = EFS_MOUNT_POINT / args.out
